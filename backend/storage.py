...
from .indexes import build_names_index, build_strings_index
from .models import Family, Person, in_memory_strings

MAGIC = "PY_GWB_01"  # magic number (8 chars)


def _encode_persons(persons: List[Person],
                    strings_map: Dict[str, int]) -> List[Dict]:
    out = []
    for p in persons:
        out.append(
            {
...
def _encode_families(families: List[Family],
                     strings_map: Dict[str, int]) -> List[Dict]:
    out = []
    for f in families:
        out.append(
...
def write_gwb(
    ...
    """Create a directory dbname.gwb with base.json,
    base.acc.json, names.inx.json, strings.inx.json.
    Inspired by GeneWeb storage (see docs)
    but JSON-encoded for Python backend.
    """
    ...
    base_content = {
        ...
        "offsets": {
            # Logical offsets (array start positions by index),
            # JSON doesn't need byte offsets
    ...
    (db_dir / "base.acc.json").write_text(
        json.dumps(base_acc, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    (db_dir / "strings.inx.json").write_text(
        json.dumps(strings_idx, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    (db_dir / "names.inx.json").write_text(
        json.dumps(names_idx, ensure_ascii=False, indent=2), encoding="utf-8"
    )
